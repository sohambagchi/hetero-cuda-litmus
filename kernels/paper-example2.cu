// kernels/paper-example2.cu — Paper Example 2 (het)
// Thread 0: FENCE_0(), write x=1, write y=1
// Thread 1: r0 = read y, FENCE_1(), r1 = read z
// Thread 2: FENCE_2(), write z=1, write a=1
// Thread 3: r2 = read a, r3 = read x
  ...
  if (shuffled_workgroup < kernel_params->testing_workgroups) {
    ...
    if (true) {
      ...
      het_spin(&het_barriers[wg_offset + id_2], HET_BARRIER_EXPECTED,
               kernel_params->barrier_spin_limit);
      FENCE_2();
      test_locations[z_2].store(1, store_order); // write z=1
...
#else
    uint r2 = test_locations[a_addr].load(cuda::memory_order_relaxed);
