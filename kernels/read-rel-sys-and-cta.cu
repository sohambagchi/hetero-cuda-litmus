// kernels/read-rel-sys-and-cta.cu — Read Release System + CTA scope test (het)
// Thread 0: store x=1 (relaxed), store y=1 (release)
// Thread 1: store y=2 (relaxed), r0 = load x (relaxed, CTA scope on GPU)
...
__global__ void litmus_test(
  ...
  if (shuffled_workgroup < kernel_params->testing_workgroups) {
    ...
    // Hardcoded: always inter-block TB_0_1
    ...
    if (true) {
      ...
      // CTA-scoped load — the key difference from read-rel-sys
...
void cpu_test_thread(
  ...
  for (int inst = start_instance; inst < end_instance; inst++) {
    ...
    // CPU has no CTA scope — just use relaxed system scope
