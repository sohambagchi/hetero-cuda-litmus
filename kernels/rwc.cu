// kernels/rwc.cu — Read-Write Causality (het)
// Thread 0: store x=1
// Thread 1: r0 = load x, FENCE_1(), r1 = load y
// Thread 2: store y=1, FENCE_2(), r2 = load x
// Weak behavior: r0==1 && r1==0 && r2==0
  ...
  if (shuffled_workgroup < kernel_params->testing_workgroups) {
    ...
    if (true) {
      ...
      het_spin(&het_barriers[wg_offset + id_2], HET_BARRIER_EXPECTED,
               kernel_params->barrier_spin_limit);
      // Thread 2: store y=1, fence, r2 = load x
...
void cpu_test_thread(
  ...
  for (int inst = start_instance; inst < end_instance; inst++) {
    ...
    // Thread 1: r0 = load x, fence, r1 = load y
#if defined(STORE_SC) || defined(LOAD_SC) || defined(THREAD_2_FENCE)
    uint r0 = test_locations[x_addr].load(cuda::memory_order_acquire);
#else
    uint r0 = test_locations[x_addr].load(cuda::memory_order_relaxed);
#endif
#if defined(BOTH_FENCE) || defined(THREAD_1_FENCE_STORE_SC) || defined(THREAD_1_FENCE_LOAD_SC)
...
#endif
#if defined(BOTH_FENCE) || defined(THREAD_2_FENCE)
    cuda::atomic_thread_fence(cuda::memory_order_seq_cst, cuda::thread_scope_system);
#endif
#if defined(LOAD_SC)
    uint r2 = test_locations[x_addr].load(cuda::memory_order_seq_cst);
#else
