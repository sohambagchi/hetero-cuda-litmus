// kernels/store.cu — Store-after-read race (het)
// Thread 0: store x=1, store y=1
// Thread 1: r0 = load y, then store x=2
// Weak behavior: x==1 && r0==1
...
__global__ void litmus_test(
  ...
  if (shuffled_workgroup < kernel_params->testing_workgroups) {
    ...
    if (true) {
      ...
      test_locations[x_1].store(2, cuda::memory_order_relaxed);
...
int host_check_results(TestResults* results, bool print) {
  if (print) {
    std::cout << "x=1, r0=1 (weak): " << results->weak << "\n";
